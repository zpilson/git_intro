# CS_372: SOCKETS AND HTTP - Program 2 (GET the data for a large file)
# Zachary Pilson
...
HTTP_COMPLIANT = "GET /wireshark-labs/HTTP-wireshark-file3.html HTTP/1.1\r\nHost:gaia.cs.umass.edu\r\n\r\n"
SIZE = 1024
...
def requestData(sockObj, http_compliant):
    ...
    # Do-While Loop: for retrieving large (or small) amounts of data until data recieved is zero
    data = sockObj.recv(SIZE)
    response = data
    while (len(data) > 0):
        data = sockObj.recv(SIZE)
        response += data
